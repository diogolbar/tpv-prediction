        ...
        if hasattr(self.model, "predict_proba"):  # Classification model
            ...
            if multi == False:
                ...
                roc_auc = roc_auc_score(
                    y, p, multi_class=multi_class, average="weighted"
                )
            ...
            return {
                ...
                "RMSE": RMSE,
                "MAE": MAE,
                "MAE/Mean(%)": round(100 * MAE / y.mean(), 2),
                "RMSE/STD (%)": round(100 * RMSE / y.std(), 2),
            }
...
def split_data(df, target_column, train_frac=0.7, val_frac=0.5, stratified=False):
    ...
    if stratified == True:
        ...
        X_train, X_val, y_train, y_val = train_test_split(
            X, y, train_size=val_frac, random_state=42
        )
