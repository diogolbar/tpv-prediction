...
def optimize_model(
    ...
    res = forest_minimize(
        ...
        space,
        random_state=42,
        n_random_starts=parameters["random_starts"],
        n_calls=parameters["calls"],
        verbose=0,
        base_estimator=parameters["estimator"],
    )
    best_params = res.x
...
def train_model(
    X_train: pd.DataFrame, y_train: pd.Series, hyperparameters: List
) -> LGBMClassifier:
    """
